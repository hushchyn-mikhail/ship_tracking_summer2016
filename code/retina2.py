...
class RetinaTrackReconstruction(object):
    ...
    def fit(self, ends_of_strawtubes, dists):
        ...
        track1 = self.conjugate_gradient_method(best_dot, tubes_starts, tubes_directions, matrixes, dists)[-1]
        ...
        if len(initial_dots)>0:
            ...
            buf = self.sigma_from
            self.sigma_from = 200
            track2 = self.conjugate_gradient_method(np.array([0, 0, 0, 0]), tubes_starts[mask], tubes_directions[mask], \
                                                    matrixes[mask], dists[mask])[-1]
            self.sigma_from = buf
